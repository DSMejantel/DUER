...
SELECT 
    'select'    as type,
    'Responsable :' AS label, 'resp' AS name, 'Choisir' as empty_option, json_group_array(json_object("label" , prenom||' '||nom, "value", username )) as options FROM (select * FROM user_info WHERE username<>'duer_admin' ORDER BY nom ASC);

-- Titre
select 'divider' as component,
      'Fiches-actions pour : '||prenom||' '||nom as contents
     FROM user_info WHERE username=:resp;  
