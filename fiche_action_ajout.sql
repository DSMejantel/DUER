...
SELECT 
    ...
    SELECT TRUE as required, 'description' as name, 'Description' as label, 'textarea' as type, 12 as width;
    SELECT TRUE as required, 'Responsable' AS label, 'resp' AS name, 'select' as type, 4 as width, 'Choisir' as empty_option, json_group_array(json_object("label" , prenom||' '||nom, "value", username )) as options FROM (select * FROM user_info WHERE username<>'duer_admin' ORDER BY nom ASC);
    SELECT TRUE as required, 'Avancement' AS label, 'av' AS name, 'select' as type, 3 as width, 'Choisir' as empty_option, '[{"label": "0 %", "value": 0}, {"label": "10 %", "value": 10}, {"label": "20 %", "value": 20},{"label": "30 %", "value": 30}, {"label": "40 %", "value": 40}, {"label": "50 %", "value": 50}, {"label": "60 %", "value": 60}, {"label": "70 %", "value": 70},{"label": "80 %", "value": 80}, {"label": "90 %", "value": 90}, {"label": "100 %", "value": 100}]'  as options;
