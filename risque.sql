...
SELECT
    '[
    ![](/icons/alert-square-'||risque.color||'.svg)](risque.sql "'||score||'/100")' as Niveau,
    lieu as Unité,
    nature as Risque,
    ...
    FROM risque LEFT JOIN unite on risque.unite_id=unite.id LEFT JOIN risques on risque.type_id=risques.id LEFT JOIN actions on actions.risque_id=risque.id LEFT JOIN user_info on actions.responsable_id=user_info.username GROUP BY risque.id ORDER BY score DESC;
